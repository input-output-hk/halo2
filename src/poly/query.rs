...
/// {A_i(X)}_i of degree n, such that A(X) := sum_i A_i(X) * X^n. In that case,
/// the `Chopped` representation of the commitment includes commitments to all
/// the pieces [A_i(X)] as well as the piece-degree `n`.
...
impl<'com, F: PrimeField, CS: PolynomialCommitmentScheme<F>> CommitmentReference<'com, F, CS> {
    ...
    ///
    /// If the commitment is parts ({com_i}_i, n), given evaluation point `x`,
    /// this function returns a vector {(x^{n * i}, com_i)}_i, representing
    /// the evaluation of this commitment at `x`.
    ///
    /// # Panics
    ///
